...
public class Util {
  public static boolean useGrayCode = false;
   ...
   public static int[] getIntArray(Array a, int intSize, boolean useGray) {
    ...
    if (intSize == speco.integer.Util.INTEGER_SIZE) {
      for (int i = 0; i < n; i++) {
        if (useGray) { intVal[i] = speco.integer.Util.grayToBinary(a.getData()[i]);
  ...
  public static byte[] getByteArray(Array a) {
    int byteSize = 8;
    int maxByte = 255;
    int pack = speco.integer.Util.INTEGER_SIZE / byteSize;
    int nInt = (a.size() + speco.integer.Util.INTEGER_SIZE - 1) / speco.integer.Util.INTEGER_SIZE;
    int n = pack * nInt;
  ...
  public static int[] getIntArray(Array a) {
    return getIntArray(a, speco.integer.Util.INTEGER_SIZE, useGrayCode);
  }
  ...
  public static void setIntArray(Array a, int[] intVal, int intSize, boolean useGray) {
    ...
    if (intSize == speco.integer.Util.INTEGER_SIZE) {
      ...
      for (int i = 0; i < n; i++) {
        if (useGray) {
          a.getData()[i] = speco.integer.Util.binaryToGray(intVal[i]);
        } else {
          a.getData()[i] = intVal[i];
        }
      }
    } else {
      n = (n * intSize) / speco.integer.Util.INTEGER_SIZE + 1;
      a.setData(new int[n]);
      int start = 0;
      for (int i = 0; i < n; i++) {
  ...
  public static int getNumber(Array a, int start, int length) {
    ...
    return (b.getInt(0) >>> (speco.integer.Util.INTEGER_SIZE - length));
  }
