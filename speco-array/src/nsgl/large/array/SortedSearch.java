package nsgl.large.array;

import nsgl.large.L2HOrder;
import nsgl.large.Order;

public class SortedSearch {
	protected Order order;
	protected long[] sorted;
	
    /**
     * Creates a search operation for the given sorted array
     * @param sorted Array of elements (should be sorted)
     */
	public SortedSearch(long[] sorted){ this( sorted, new L2HOrder() ); }

    /**
     * Creates a search operation for the given sorted array
     * @param sorted Array of elements (should be sorted)
     */
	public SortedSearch(Array sorted){ this( sorted.buffer ); }
     ...
     */
	public SortedSearch(long[] sorted, Order order){
		this.order = order;
		this.sorted = sorted;
	}
	
	public void set( long[] sorted ){ this.sorted = sorted; } 
     ...
     */
    public int find(long x) { return find( 0, sorted.length, x ); }
     ...
     * @return The position of the given object, -1 if the given object is not in the array
     */
    public int find(int start, int end, long x) {
     ...
     */
    public boolean contains(int start, int end, long x){ return (find(start, end, x) != -1); }
     ...
     * @return <i>true</i> if the element belongs to the sorted array, <i>false</i> otherwise
     */
    public boolean contains(long x) { return (find(x) != -1); }
     ...
     */
    public int findRight(long x) { return findRight( 0, sorted.length, x ); }
     ...
     * @return Position of the object that is bigger than the given element
     */
    public int findRight(int start, int end, long x) {
        if (end > start) {
            int a = start;
            int b = end - 1;
     ...
     */
    public int findLeft(long x){ return findLeft( 0, sorted.length, x ); }
     ...
     * @return Position of the object that is smaller than the given element
     */
    public int findLeft(int start, int end, long x) {
        if (end > start) {
            int a = start;
            int b = end - 1;
