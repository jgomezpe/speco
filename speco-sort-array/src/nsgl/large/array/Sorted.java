...
public class Sorted extends DynArray{
    ...
    public Sorted( long[] buffer, int n, Order order ){
        super( buffer, n );
        this.order = order;
        search = new SortedSearch(buffer, order);
    }

    public Sorted( long[] buffer, Order order ){
        super( buffer );
        this.order = order;
        search = new SortedSearch(buffer, order);
    }
    ...
    public boolean set( int index, long data ) throws IndexOutOfBoundsException{
    ...
    public boolean add( int index, long data ) throws IndexOutOfBoundsException{
        if( 0 <= index && index <= size ){
        	boolean flag = 	(index==0 || order.compare(get(index-1), data)<=0) &&
